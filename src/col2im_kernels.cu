...
__global__ void col2im_gpu_kernel(const int n, const float* data_col,
    ...
    for(; index < n; index += blockDim.x*gridDim.x){
        ...
        // 这里可能有点问题
        int offset =
            (c * ksize_h * ksize_w + h * ksize_h + w) * height_col * width_col;
        int coeff_h_col = (1 - stride_h * ksize_h * height_col) * width_col;
...
void col2im_gpu(float *data_col,
    ...
    col2im_gpu_kernel<<<(num_kernels+BLOCK-1)/BLOCK,
        BLOCK>>>(
                num_kernels, data_col, height, width, ksize,ksize,pad,pad,
                stride, stride, height_col,
                width_col, data_im);
}
...
col2im_gpu_kernel<<<(num_kernels+BLOCK-1)/BLOCK,
    BLOCK>>>(
            num_kernels, data_col, height, width, ksize_h,ksize_w, pad_h,pad_w,
